...
func _ready() -> void:
	...
	_atlas = AssetLoader.load_texture(base_tex.resource_path) if not Engine.is_editor_hint() else base_tex
	if _atlas == null:
		_atlas = base_tex
...
func _draw() -> void:
	...
	var total_w := glyphs.size() * CHAR_SIZE
	var x_offset := -total_w / 2.0
	for idx in glyphs.size():
		...
		draw_texture_rect_region(_atlas, Rect2(x_offset + idx * CHAR_SIZE, 0, CHAR_SIZE, CHAR_SIZE), src)
